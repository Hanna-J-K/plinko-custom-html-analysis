...
const checkForBootstrapImports = ($) => {
    for(const element of $(globalSelector + 'link')) {
        if ($(element).attr('href').includes('bootstrap'))
            return { result : "PASS", value : $(element).attr('href') };
    }
    return { result : "FAIL" };
}
...
const checkForAEMGridClasses = ($) => {
    if ($(globalSelector + '*[class*="aem-Grid"]').length > 0) {
        return { result : "PASS", value : $(globalSelector + '*[class*="aem-Grid"]').attr('class')};
    }
    return { result : "FAIL" };
}
...
const hasImgWithoutLazyLoading = ($) => {
    if ($(globalSelector + 'img[class*="lazy-image"]').length === 0) {
        return { result : "PASS", value : $(globalSelector + 'img[class*="lazy-image"]').length};
    }
    return { result : "FAIL" };
}
...
const hasImgWithoutAlt = ($) => {
    if ($(globalSelector + 'img:not([alt])').length > 0) {
        return { result : "FAIL", value : $(globalSelector + 'img:not([alt])').length};
    }
    return { result : "PASS" };
}
...
const hasStyleAttribute = ($) => {
    if ($(globalSelector + '*[style]').length > 0) {
        return { result : "FAIL", value : $(globalSelector + '*[style]').attr('style')};
    }
    return { result : "PASS" };
}
